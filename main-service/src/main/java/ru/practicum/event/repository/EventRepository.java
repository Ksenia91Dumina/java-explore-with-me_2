...
public interface EventRepository extends JpaRepository<Event, Long>, JpaSpecificationExecutor<Event>,
    ...
    Page<Event> findAllByInitiatorId(Long initiatorId, PageRequestOverride pageRequest);
    ...
    List<Event> findByInitiatorAndCategoriesAndState(List<Long> users, List<Long> categories,
                                                     List<EventState> states, PageRequestOverride pageRequest);

    @Query("SELECT e FROM Event AS e WHERE (e.state IN :states)")
    List<Event> findByState(List<EventState> states, PageRequestOverride pageRequest);

    @Query("SELECT e FROM Event AS e " +
        "WHERE (e.category.id IN :categories) AND (e.state IN :states)")
    List<Event> findByCategoriesAndState(List<Long> categories, List<EventState> states, PageRequestOverride pageRequest);

    @Query("SELECT e FROM Event AS e " +
        "WHERE (e.initiator.id IN :users) AND (e.state IN :states)")
    List<Event> findByInitiatorAndState(List<Long> users, List<EventState> states, PageRequestOverride pageRequest);
